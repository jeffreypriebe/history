...
export const hasBasename = (path, prefix) => 
  matchBasename(path, prefix).match
...
export const parseBasepath = (path, prefix) => {
  const { match, keys } = matchBasename(path, prefix)

  if (!match) return prefix

  let basepath = prefix
  keys.forEach((key, index) => {
    const matchKey = new RegExp(`:${key.name}(\\/|$|\\?|#)`,'gi')
    basepath = basepath.replace(matchKey, match[index + 1])
  })
